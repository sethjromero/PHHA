...
elev_rast <- get_elev_raster(locations = elev_window, z = 5, clip = 'tile') %>%
  rast() %>%
...
```{r}
grin <- read.csv('../../ExSitu_v_InSitu/KRLA/GRIN_KRLA_coord.csv') %>%
...
elelAll <- read.csv('../../ExSitu_v_InSitu/KRLA/GRIN_SOS_coord.csv') %>%
  filter(str_sub(Taxon, 1, 3) == 'Ely') %>%
  rename(Lat = Latitude,
         Long = Longitude) %>%
  select(Accession, Lat, Long) %>%
  na.omit() %>%
  prjPoints()

wild <- read.csv('../../ExSitu_v_InSitu/KRLA/KRLA_latlong_formap.csv') %>%
  rename(Lat = lat,
         Long = long) %>%
  prjPoints()
```
...
phhaMap <- baseMap +
  new_scale_fill() +
  geom_point(data = elelAll, aes(x = prjx, y = prjy), 
             size = 4, 
             color = 'black', 
             fill = 'red', 
             pch = 21)
  # geom_point(data = wild, aes(x = prjx, y = prjy), 
  #            size = 4, 
  #            color = 'black', 
  #            fill = 'yellow', 
  #            pch = 21)
